...
Instead of a straight line polymer chain, gives out a self avoiding
random walk configuration for the polymer chain.

This file gives different molecule ID for each chain. Charge percentage, absolute charge and 
initial density can be set before making the model.

This file creates separate positive and negative charges. Positive charges are on the backbone
and negative charges exist as separate beads that aren't part of the polymer (free ions).
"""
...
while numberofchains2!=0:
    ...
    q+=3
    molID+=1
#FREE IONS are added here.
atomid_ion=starta -atomnum + actual_moves
independent_ions=0
while independent_ions<count:
    '--This part creates one chain--'
    xtemp, ytemp, ztemp=random.uniform(xstart,xend), random.uniform(ystart,yend), random.uniform(zstart,zend)
    curr_bin = np.digitize(np.array([xtemp,ytemp,ztemp]),edges[0])
    if str(curr_bin) not in all_bins:
        all_bins[str(curr_bin)]=[]
    all_bins[str(curr_bin)].append([xtemp,ytemp,ztemp])
    ...
    if all_atoms_far:
        ...
        yy = ytemp
        zz = ztemp
        atoms.append([atomid_ion, molID, 3, -charge, xx, yy, zz])
        molID+=1
        independent_ions+=1
...
f=open('CG_%sC_%sDP_d0p%s_cp%s_cv%s-ions-separate.data' %(numberofchains,monomernum,int(density*100),charge_percent,charge),'w')
