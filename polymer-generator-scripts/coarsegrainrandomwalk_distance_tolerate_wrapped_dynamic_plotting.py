...
while numberofchains2!=0:
    ...
    yy_prev = yy
    zz_prev = zz
    ax.scatter3D(xx, yy, zz, c=colors[numberofchains2%10], cmap='Greens')
    plt.draw()
    plt.pause(1e-17)
    ...
    while actual_moves<atomnum:
        ...
        if all_atoms_far:
            ...
            y_values = [yy_prev, yy]
            z_values = [zz_prev, zz]
            if ((xx - xx_prev)**2 + (yy - yy_prev)**2 + (zz-zz_prev)**2)**(0.5) <1.1:
                ax.plot(x_values,y_values,z_values,c=colors[numberofchains2%10])
            plt.draw()
            plt.pause(1e-50)
            if (i+1)%11!=0:
