...
Modified on Fri Jan 18 2019 by @prathameshraiter
...
length=1.3 # average bond length
...
while boxsize:
    ...
    while numberofchains2!=0:
        xx, yy, zz=random.randrange(-20,20), random.randrange(-20,20), random.randrange(-20,20)
        ...
        for i in range(starta, starta+atomnum):
            ...
            xx += length*length*(sin(theta)*cos(phi))
            yy += length*length*(sin(theta)*sin(phi))
            zz += length*length*(cos(theta))
...
f=open('CG_%sC_%sDP_sizelimits.data' %(numberofchains,monomernum),'w')
